...
public class AuthenticationController {
	...
	private String processRegisterForm(@ModelAttribute @Valid RegisterFormDTO registerFormDTO,
	                                   Errors errors,
	...
	@PostMapping("/login")
	private String processLoginForm(@ModelAttribute @Valid LoginFormDTO loginFormDTO,
	                                Errors errors,
	                                HttpServletRequest request,
	                                Model model) {
		if (errors.hasErrors()) {
		...
		if (theUser == null) {
			errors.rejectValue("username", "user.invalid", "User does not exist.");
			model.addAttribute("title", "Login");
			return "login";
		}
		...
		if (!theUser.isMatchingPassword(password)) {
			errors.rejectValue("password", "password.mismatch", "Password is incorrect.");
			model.addAttribute("title", "Login");
			return "login";
		}
